...
if run_mode == 'async':
    ...
    data_baseline = df.query(f'{baseline_after} <= time_idx <= {baseline_before}').transpose().values
    data_highlight = df.query(f'{highlight_after} <= time_idx <= {highlight_before}').transpose().values
